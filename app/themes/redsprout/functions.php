...
add_action('rest_api_init', 'wpbasic_register_endpoints');
...
function wpbasic_enqueue_scripts()
{
    ...
    wp_localize_script('redsprout', 'wpbasic_vars', array(
 ...
 * Rest endpoint
 */
function wpbasic_register_endpoints()
{
    register_rest_route(
        'redsprout/v1',
        '/votes/',
 ...
 */
function wpbasic_add_vote(WP_REST_Request $request)
{
    ...
    if (false === (bool)update_post_meta($request->get_param('id'), 'votes', $votes + 1)) {
        return new WP_Error('vote_error', __('Unable to add vote', 'wpbasic'), $request->get_param('id'));
    }
    return $votes + 1;
